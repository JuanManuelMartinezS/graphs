// Agregar estas funciones auxiliares al componente
export const clearDistanceRoutes = () => {
    // Limpiar rutas
    distanceRoutesRef.current.forEach(route => {
      if (route && mapInstance.current) {
        mapInstance.current.removeControl(route);
      }
    });
    distanceRoutesRef.current = [];
    ...
    if (distanceLegendRef.current) {
      distanceLegendRef.current.remove();
      distanceLegendRef.current = null;
    }
  };
  
  const highlightStartNode = (node) => {
    ...
    distanceMarkersRef.current.push(startMarker);
  };
  ...
  export const createColoredRoute = (startNode, endNode, color, distance) => {
    ...
    orsDirections.addTo(mapInstance.current);
    distanceRoutesRef.current.push(orsDirections);
    ...
    distanceMarkersRef.current.push(distanceLabel);
  };
  ...
  export const showDistanceLegend = (distances) => {
    ...
    distanceLegendRef.current = legend;
  };